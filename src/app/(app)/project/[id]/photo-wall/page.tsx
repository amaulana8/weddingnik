...
export default async function PhotoWallPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: items } = await supabase.from('guestbook_entries').select('*').eq('tenant_id', id).order('created_at', { ascending: false })
  const itemCount = items?.length || 0

  return (
    <div className="animate-fade-in">
      <div className="mb-6">
        <h1 className="text-lg font-semibold text-navy-800 tracking-tight">Photo Wall</h1>
