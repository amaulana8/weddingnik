    ...
    <div className="animate-fade-in space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-xs text-navy-400/60 mt-0.5">Digital wedding invitation</p>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          {inv && <CopyLinkButton url={`/invitation/${id}`} />}
          <Link href={`/invitation/${id}`} target="_blank" className="h-8 px-3 rounded-lg bg-white border border-navy-100/20 text-navy-500 text-xs font-semibold flex items-center gap-1.5 hover:bg-navy-50/40 transition-all">
